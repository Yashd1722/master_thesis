...
FIX: Correctly computes AUC by comparing Forced vs Neutral segments (Bury Protocol).
"""
import argparse
import json
import logging
...
def experiment_dir(model, dataset, metric, cfg) -> Path:
    name = cfg["naming"]["experiment_dir"].format(model=model, dataset=dataset, metric=metric)
...
def load_dl_models(model_name, dataset_name, ts_len, num_classes, cfg, device, logger):
    ...
    for v in pad_variants:
        name = cfg["naming"]["checkpoint_dl"].format(model=model_name, dataset=dataset_name, variant=v)
        fpath = ckpt_dir / name
        if not fpath.exists():
            logger.warning(f"  Not found: {name}"); continue
    ...
    if not models: raise RuntimeError(f"No checkpoints for {model_name}/{dataset_name}")
    return models

def load_tsc_model(model_name, dataset_name, cfg, logger):
    import joblib
    ckpt_dir = REPO_ROOT / cfg["paths"]["checkpoints"]
    name = cfg["naming"]["checkpoint_tsc"].format(model=model_name, dataset=dataset_name)
    fpath = ckpt_dir / name
    if not fpath.exists(): raise RuntimeError(f"TSC checkpoint not found: {name}")
...
def evaluate_zenodo(model_name, dataset_name, cfg, device, logger):
    ...
    ts_len, num_classes = ds_info["ts_length"], ds_info["num_classes"]
    ...
    if is_tsc_model(model_name):
        ...
        models = load_dl_models(model_name, dataset_name, ts_len, num_classes, cfg, device, logger)
    ...
    X_all, y_all = [], []
    for x, y in test_dl:
        X_all.append(x.squeeze(1).numpy()); y_all.append(y.numpy())
    X_all, y_all = np.concatenate(X_all), np.concatenate(y_all)

    if is_tsc_model(model_name):
        stds = X_all.std(axis=1)
        valid_mask = stds > 1e-7
        if (~valid_mask).sum() > 0: logger.warning(f"  Dropping {(~valid_mask).sum()} flat test series")
        X_all, y_all = X_all[valid_mask], y_all[valid_mask]
        
    if len(X_all) == 0: logger.error("  No valid samples remaining."); return
    ...
    base = {"model": model_name, "dataset": dataset_name, "num_classes": num_classes, 
             "class_names": class_names, "inference_time_sec": round(time.time() - t0, 2),
             "timestamp": time.strftime("%Y-%m-%dT%H:%M:%S")}

    auc_dir = experiment_dir(model_name, dataset_name, "auc", cfg)
    auc_data = {**base, "metric": "auc", "auc": round(auc, 6), "roc_fpr": fpr, "roc_tpr": tpr, "roc_thresh": thresh}
...
# =============================================================================
# Evaluate PANGAEA (Bury Protocol: Forced vs Neutral)
# =============================================================================
    ...
    for core_name in cores:
        ...
        for sap_id, sap_res in sap_results.items():
            for element, elem_res in sap_res.items():
                ...
                if not forced_res or not forced_res.get("dl_inputs"): continue
                
                # 1. Inference on Forced (Positive Class)
                X_forced = np.stack(forced_res["dl_inputs"])
                probs_forced = predict_fn(X_forced)
                p_trans_forced = (1.0 - probs_forced[:, null_idx] if null_idx >= 0 else probs_forced[:, -1])
                ...
                if neutral_res and neutral_res.get("dl_inputs"):
                    ...
                    p_trans_neutral = (1.0 - probs_neutral[:, null_idx] if null_idx >= 0 else probs_neutral[:, -1])
                
                # 3. Compute AUC (Forced vs Neutral) -> THIS FIXES THE NaN!
                auc = float('nan')
                fpr, tpr = [0, 1], [0, 1]
                if len(p_trans_neutral) > 0:
                    y_true = np.concatenate([np.ones(len(p_trans_forced)), np.zeros(len(p_trans_neutral))])
                ...
                base = {"model": model_name, "dataset": dataset_name, "core": core_name, 
                         "sapropel": sap_id, "element": element, "n_forced": len(p_trans_forced),
                         "n_neutral": len(p_trans_neutral), "p_transition": p_trans_forced.tolist(),
                         "timestamp": time.strftime("%Y-%m-%dT%H:%M:%S")}
                
                auc_dir = experiment_dir(model_name, f"pangaea_{tag}", "auc", cfg)
                auc_data = {**base, "metric": "auc", "auc": round(auc, 6), "roc_fpr": fpr, "roc_tpr": tpr}
                save_result(auc_data, auc_dir)
                
                ktau_dir = experiment_dir(model_name, f"pangaea_{tag}", "kendall_tau", cfg)
                ktau_data = {**base, "metric": "kendall_tau", "kendall_tau": round(tau, 6),
                              "variance": forced_res.get("variance", []), "lag1_ac": forced_res.get("lag1_ac", [])}
                save_result(ktau_data, ktau_dir)
                
                plot_roc(auc_data, auc_dir)
                
                class SimpleResult:
                    def __init__(self, var, ac): self.variance = var; self.lag1_ac = ac
                simple_result = SimpleResult(forced_res.get("variance", []), forced_res.get("lag1_ac", []))
...
def main():
    ...
    logger = setup_log(log_dir / f"{args.model}_{args.dataset}_{args.target}_eval.log")

    logger.info(f"Model: {args.model} | Dataset: {args.dataset} | Target: {args.target}")

    if args.target == "zenodo": evaluate_zenodo(args.model, args.dataset, cfg, device, logger)
    else: evaluate_pangaea(args.model, args.dataset, cfg, device, logger)
